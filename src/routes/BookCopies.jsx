...
export default function BookCopies() {
  ...
  const fetchBookCopies = async () => {
    ...
    try {
      const data = await fetchData(
        `/public/bookCopy?title=${encodeURIComponent(title)}&page=${page}&barcode=${barcodeFilter}&libraryName=${libraryFilter}`,
        "GET"
      );
      if (data.success) {
        setCopies((prev) => {
          const newCopies = data.message.filter((c) => !prev.some((p) => p.id === c.id));
  ...
  const handleAddCopy = async () => {
    ...
    try {
      ...
      await fetchData(
        "/librarian/bookCopy",
        "POST",
        { bookTitle: title, barcode: newCopy.barcode, libraryName: newCopy.libraryName },
        token
      );
  ...
  const handleUpdate = async () => {
    if (!selectedLibrary && !selectedBarcode) {
      toast.error("At least one attribute (Barcode or Library) must be updated");
      return;
    }
    try {
      const token = localStorage.getItem("token");
      await fetchData(
        `/librarian/bookCopy`,
        "PUT",
        { id: selectedCopyId, newLibrary: selectedLibrary || "", newBarcode: selectedBarcode || "" },
        token
      );
  ...
  const handleDeleteCopy = async () => {
    try {
      const token = localStorage.getItem("token");
      await fetchData(`/librarian/bookCopy?copyId=${selectedCopyId}`, "DELETE", null, token);
  ...
  if (error) {
    return (
      <div className="container mt-5 alert alert-danger" role="alert" aria-live="assertive">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="container mt-5">
      <div className="d-flex justify-content-between align-items-center">
        ...
        <Button variant="primary" onClick={() => setShowModal(true)} aria-label="Add New Copy">
          Add New Copy
        </Button>
      </div>

      <div className="mt-3 d-flex align-items-center flex-wrap" role="region" aria-label="Filters">
        <input
          type="text"
          name="barcode"
          id="barcode"
      ...
      <InfiniteScroll
        ...
        loader={<Loading />}
        endMessage={<p className="text-center my-2" role="status">There aren't more copies</p>}
      >
          ...
          {copies.map((copy) => (
            ...
            >
              <div className="col-md-2">
                ...
                <span aria-labelledby={`barcode-label-${copy.id}`}>{copy.barcode}</span>
              </div>
              <div className="col-md-2">
                <strong id={`available-label-${copy.id}`}>Available:</strong>{" "}
                <span
                  aria-labelledby={`available-label-${copy.id}`}
                  style={{ color: copy.loanUser === "none" ? "#198754" : "#dc3545" }}
                >
              ...
              <div className="col-md-3">
                ...
                <span aria-labelledby={`library-label-${copy.id}`}>{copy.libraryName}</span>
              </div>
              <div className="col-md-3">
              ...
              <div className="col-md-2 text-end d-flex justify-content-between">
                <Button
                  ...
                  size="sm"
                  className="me-1"
                  onClick={() => handleChangeLibrary(copy.id, copy.barcode, copy.libraryName)}
